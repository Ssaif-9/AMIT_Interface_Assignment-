...
void UART_init (void)
{
	...
	UBRRL=103;             // Set baud rate = 9600 -> 103;
...
void UART_ReceiveString(u8* ReceiveBuffer, u8 BufferSize)
{
	if (ReceiveBuffer != NULL && BufferSize > 0)
	{
		u8 receivedChar;
		u8 index = 0;

		// Receive characters until either BufferSize-1 characters are received or a newline '\n' is received.
		while (index < BufferSize - 1)
		{
			while (0 == GET_BIT(UCSRA, RXC)); // Wait for data to be available

			receivedChar = UDR; // Read the received character

			// Check for termination condition (newline or null terminator)
			if (receivedChar == '\n' || receivedChar == '\0')
			{
			...
			ReceiveBuffer[index] = receivedChar;
			index++;
		}
